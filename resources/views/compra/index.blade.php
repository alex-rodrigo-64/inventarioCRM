...
@section('content')
<h1 align="center" style="font-weight: 700">COMPRAS REALIZADAS</h1>
  ...
  <br>
    ...
    <div class="tabla-general" >
        ...
        <thead class="table table-striped table-bordered text-white" style="background:rgb(2, 117, 216); color: aliceblue">
            <tr>
            <th class="column1 text-center"># Compra</th>
            <th class="column2 text-center">Sucursal</th>
            <th class="column3 text-center">Almacen</th>
            <th class="column3 text-center">Proveedor</th>
        ...
        <tbody class="table-bordered" id="myTable"> 
            ...
            <tr>
                ...
                <td class="text-center" >{{$compra->id_sucursal}}</td>
                <td class="text-center" >{{$compra->id_almacen}}</td>
                <td class="text-center" >{{$compra->proveedor}}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($compra->created_at)->format('d-m-Y')}}</td>
                ...
                @can('mostrar venta')
                     {{-- MOSTRAR --}}
                <a href="{{ url('/compra/mostrar/'.$compra->id)}}">
                ...
                @can('eliminar producto')
                  {{-- ELIMINAR --}}
                  <button class="btn d-inline" style="color: red"  data-toggle="modal" data-target="#eliminar{{$compra->id}}">
                ...
                @endcan
                  ...
                  {{-- ELIMINAR --}}
                  <div class="modal fade" id="eliminar{{$compra->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title w-100 text-center" id="exampleModalLabel">Eliminar Compra</h5>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                          </button>
                        </div>
                        <div class="modal-body">
                         ¿Realmente Desea Borrar Esta Compra? <br>
                         (Se Descontara de su Inventario la Compra Realizada)
                        </div>
